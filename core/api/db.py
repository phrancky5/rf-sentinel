...
def save_bookmark(bk_id: str, label: str, freq_mhz: float, notes: str = "") -> None:
    if not _conn:
        return
    from datetime import datetime, timezone
    with _lock:
        _conn.execute(
            """INSERT INTO bookmarks (id, label, freq_mhz, notes, created_at)
               VALUES (?, ?, ?, ?, ?)""",
            (bk_id, label, freq_mhz, notes, datetime.now(timezone.utc).isoformat()),
        )
        _conn.commit()
    logger.info("Saved bookmark %s: %s @ %.3f MHz", bk_id[:8], label, freq_mhz)
...
def list_bookmarks() -> list[dict]:
    if not _conn:
        return []
    rows = _conn.execute(
        "SELECT id, label, freq_mhz, notes, created_at FROM bookmarks ORDER BY freq_mhz",
    ).fetchall()
    return [dict(r) for r in rows]


def update_bookmark(bk_id: str, label: str, freq_mhz: float, notes: str = "") -> bool:
    if not _conn:
        return False
    with _lock:
        cur = _conn.execute(
            "UPDATE bookmarks SET label = ?, freq_mhz = ?, notes = ? WHERE id = ?",
            (label, freq_mhz, notes, bk_id),
        )
        _conn.commit()
    updated = cur.rowcount > 0
    if updated:
        logger.info("Updated bookmark %s: %s @ %.3f MHz", bk_id[:8], label, freq_mhz)
    return updated


def delete_bookmark(bk_id: str) -> bool:
    if not _conn:
        return False
    with _lock:
        cur = _conn.execute("DELETE FROM bookmarks WHERE id = ?", (bk_id,))
        _conn.commit()
    deleted = cur.rowcount > 0
    if deleted:
        logger.info("Deleted bookmark %s", bk_id[:8])
    return deleted
